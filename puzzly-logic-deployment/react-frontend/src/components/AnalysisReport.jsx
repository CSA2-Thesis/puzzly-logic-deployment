...
const Cpu = React.lazy(() => import("lucide-react").then(module => ({ default: module.Cpu })));
const Trophy = React.lazy(() => import("lucide-react").then(module => ({ default: module.Trophy })));
...
export const AnalysisReport = React.memo(({ data, filteredData }) => {
  ...
  const formatAlgorithmList = React.useCallback((algorithms) => {
  ...
  const getPerformanceInsights = React.useCallback((summary) => {
    const insights = [];
  ...
  const typeIcons = {
    accuracy: <IconLoader><CheckCircle className="w-5 h-5" /></IconLoader>,
    speed: <IconLoader><Zap className="w-5 h-5" /></IconLoader>,
    memory: <IconLoader><Cpu className="w-5 h-5" /></IconLoader>,
    overall: <IconLoader><Trophy className="w-5 h-5" /></IconLoader>,
  };
    ...
    <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-[#172030] dark:to-gray-900 rounded-2xl p-8 shadow-md mb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {performanceCards.map((card, idx) => (
          <PerformanceCard key={idx} card={card} />
        ))}
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard label="Total Runs" value={summary.totalRuns} color="blue" />
        <StatCard label="Avg Accuracy" value={`${summary.avgAccuracy}%`} color="green" />
        <StatCard label="Avg Execution Time" value={`${summary.avgExecutionTime}s`} color="purple" />
        <StatCard 
          label="Top Performer" 
          value={summary.bestAlgorithms.length === 1 ? summary.bestAlgorithms[0].algorithm : "Multiple"} 
          color="orange" 
        />
      </div>
        ...
        <div className="space-y-3">
          <AnimatePresence>
            {insights.map((insight, index) => (
              <InsightItem key={index} insight={insight} typeStyles={typeStyles} typeIcons={typeIcons} />
            ))}
          </AnimatePresence>
        </div>
      </div>

      <ComparisonTable data={data} summary={summary} />
    </div>
  );
});

const PerformanceCard = React.memo(({ card }) => (
  <motion.div
    whileHover={{ scale: 1.05 }}
    transition={{ type: "spring", stiffness: 200, damping: 12 }}
    className={`relative ${card.bgClass} p-6 rounded-xl shadow-lg overflow-hidden`}
  >
    <div className={`absolute inset-0 rounded-xl ${card.glowClass} blur-xl`}></div>
...
const InsightItem = React.memo(({ insight, typeStyles, typeIcons }) => (
  <motion.div
...
const ComparisonTable = React.memo(({ data, summary }) => (
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-gray-100 dark:bg-gray-700">
          <tr>
            {['Algorithm', 'Accuracy', 'Time (s)', 'Memory', 'Runs'].map((header) => (
              <th key={header} className="px-4 py-2 text-left font-medium text-gray-600 dark:text-gray-300">
                {header}
              </th>
            ))}
          </tr>
        ...
        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
          {data.map((algo, index) => (
            <TableRow key={index} algo={algo} summary={summary} />
          ))}
        </tbody>
      </table>
    </div>
...
const TableRow = React.memo(({ algo, summary }) => {
  const isHighlighted =
  ...
  return (
    <tr className={isHighlighted ? "bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/10" : ""}>
